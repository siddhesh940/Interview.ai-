...
class CapgeminiScraper {
  constructor() {
    ...
    this.baseUrl = 'https://careers.capgemini.com';
    this.searchUrls = [
      'https://careers.capgemini.com/in-en/jobs?search_api_fulltext=&field_job_level%5B0%5D=Graduate&page=0',
      'https://careers.capgemini.com/in-en/jobs?search_api_fulltext=fresher&page=0'
    ];
  }
    ...
    try {
      ...
      for (const url of this.searchUrls) {
        try {
          ...
          await page.waitForSelector('.job-item, .job-result, .job-card, [class*="job"]', { timeout: 10000 });
          
          const drives = await page.evaluate(() => {
            const jobElements = document.querySelectorAll('.job-item, .job-result, .job-card, [class*="job"]');
            const drives = [];
            
            jobElements.forEach(element => {
              try {
                const titleElement = element.querySelector('h3, .job-title, .title, h2');
                const linkElement = element.querySelector('a');
                const locationElement = element.querySelector('.location, .job-location');
                const descElement = element.querySelector('.description, p');
                ...
                const isRelevant = /graduate|fresher|entry|trainee|associate|campus|junior/i.test(title) ||
                                 /graduate|fresher|entry|trainee|associate|campus/i.test(description);
                
                if (isRelevant) {
                  drives.push({
                    title,
                    link: link.startsWith('http') ? link : `https://careers.capgemini.com${link}`,
                    location,
                    description,
...
module.exports = CapgeminiScraper;
