...
class CognizantScraper {
  constructor() {
    ...
    this.baseUrl = 'https://careers.cognizant.com';
    this.searchUrls = [
      'https://careers.cognizant.com/in/en/search-results?keywords=&category=students-and-graduates',
      'https://careers.cognizant.com/in/en/search-results?keywords=fresher',
      'https://careers.cognizant.com/in/en/search-results?keywords=entry%20level'
    ];
  }
    ...
    try {
      ...
      for (const url of this.searchUrls) {
        try {
          ...
          await page.waitForSelector('.job-tile, .job-result, [data-ph-id="ph-page-element-page14-7QW3tP"], [class*="job"]', { timeout: 10000 });
          
          const drives = await page.evaluate(() => {
            const jobElements = document.querySelectorAll('.job-tile, .job-result, [data-ph-id*="job"], [class*="job"]');
            const drives = [];
            ...
            jobElements.forEach(element => {
              try {
                ...
                const location = locationElement?.textContent?.trim() || '';
                                ...
                                const isRelevant = /fresher|graduate|entry|trainee|associate|campus|junior|analyst|cognizant university/i.test(title) ||
                                                 /fresher|graduate|entry|trainee|associate|campus/i.test(description);
                                
                                if (isRelevant) {
                                  drives.push({
                                    title,
                                    link: link.startsWith('http') ? link : `https://careers.cognizant.com${link}`,
                                    location,
                                    description,
...
return [];
                    ...
                    for (const drive of rawDrives) {
                      try {
                        ...
                        // Cognizant typically requires 60% (6.0 CGPA)
                        const minCgpa = 6.0;
...
return false;
                ...
                module.exports = CognizantScraper;
